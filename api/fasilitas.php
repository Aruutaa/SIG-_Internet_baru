...
function bool_param($value) {
    return !empty($value) && $value !== 'false' && $value !== '0' ? 't' : 'f';
}
...
if ($method === 'GET') {
    $sql = "SELECT id, nama_faskes, jenis_faskes, kecamatan, alamat, sumber_data, catatan,
                   ...
                   bpjs, igd_24_jam, rawat_inap, ambulans,
                   kapasitas_tempat_tidur, jumlah_dokter, jumlah_tenaga_kesehatan, rating,
...
if ($method === 'POST') {
    ...
    $stmt = $pdo->prepare("INSERT INTO fasilitas_kesehatan
        (nama_faskes, jenis_faskes, kecamatan, alamat, sumber_data, catatan, status_operasional, telepon, jam_layanan, bpjs, igd_24_jam, rawat_inap, ambulans, kapasitas_tempat_tidur, jumlah_dokter, jumlah_tenaga_kesehatan, rating, geom, created_by)
        VALUES (:nama, :jenis, :kecamatan, :alamat, :sumber, :catatan, :status, :telepon, :jam, :bpjs, :igd, :rawat_inap, :ambulans, :bed, :dokter, :tenaga, :rating, ST_SetSRID(ST_MakePoint(:lng, :lat), 4326), :created_by)
        RETURNING id");
    $stmt->execute([
        ...
        'status'=>$d['status_operasional'] ?? 'Aktif', 'telepon'=>$d['telepon'] ?? '-', 'jam'=>$d['jam_layanan'] ?? '',
        'bpjs'=>bool_param($d['bpjs'] ?? false), 'igd'=>bool_param($d['igd_24_jam'] ?? false),
        'rawat_inap'=>bool_param($d['rawat_inap'] ?? false), 'ambulans'=>bool_param($d['ambulans'] ?? false),
        'bed'=>(int)($d['kapasitas_tempat_tidur'] ?? 0), 'dokter'=>(int)($d['jumlah_dokter'] ?? 0),
        'tenaga'=>(int)($d['jumlah_tenaga_kesehatan'] ?? 0), 'rating'=>(float)($d['rating'] ?? 0),
...
if ($method === 'PUT') {
    ...
    $stmt = $pdo->prepare("UPDATE fasilitas_kesehatan SET
        ...
        catatan=:catatan, status_operasional=:status, telepon=:telepon, jam_layanan=:jam,
        bpjs=:bpjs, igd_24_jam=:igd, rawat_inap=:rawat_inap, ambulans=:ambulans,
        kapasitas_tempat_tidur=:bed, jumlah_dokter=:dokter, jumlah_tenaga_kesehatan=:tenaga, rating=:rating,
        geom=ST_SetSRID(ST_MakePoint(:lng, :lat), 4326), updated_at=NOW()
