...
function normalizeFeature(feature, idx){
  ...
  return {
    ...
    desa:p.desa || '-',
    alamat:p.alamat || '-',
    latitude:Number(p.latitude ?? p.lat ?? ll?.lat),
    longitude:Number(p.longitude ?? p.lng ?? ll?.lng),
    telepon:p.telepon || '-',
    jam_layanan:p.jam_layanan || p.jam_operasional || p.jam || '-',
    bpjs:asBool(p.bpjs),
    igd_24_jam:asBool(p.igd_24_jam ?? p.igd),
    rawat_inap:asBool(p.rawat_inap),
    ambulans:asBool(p.ambulans),
    kapasitas_tempat_tidur:Number(p.kapasitas_tempat_tidur ?? p.bed ?? 0),
    jumlah_dokter:Number(p.jumlah_dokter ?? p.dokter ?? 0),
    jumlah_tenaga_kesehatan:Number(p.jumlah_tenaga_kesehatan ?? p.tenaga ?? 0),
...
function filtered(){
  ...
  return facilities.filter(f => activeTypes.has(f.jenis) && (!kec || f.kecamatan===kec) && (!bpjs || String(Number(!!f.bpjs))===bpjs) && (!igd || String(Number(!!f.igd_24_jam))===igd) && (!keyword || [f.nama,f.jenis,f.kecamatan,f.desa,f.alamat,f.layanan,f.kode].join(' ').toLowerCase().includes(keyword)));
}
...
function popupHTML(f){
  ...
  const tags = [f.bpjs?'BPJS':null,f.igd_24_jam?'IGD 24 Jam':null,f.rawat_inap?'Rawat Inap':null,f.ambulans?'Ambulans':null].filter(Boolean).map(x=>`<span class="mini-tag">${x}</span>`).join(' ');
  return `<div class="popup-title">${escapeHTML(f.nama)}</div><div class="popup-meta">${escapeHTML(f.jenis)} • ${escapeHTML(f.kecamatan)}</div><div>${tags}</div><div style="margin-top:6px"><b>Alamat:</b><br>${escapeHTML(f.alamat)}</div><div style="margin-top:6px"><b>Jam:</b> ${escapeHTML(f.jam_layanan)}<br><b>Dokter:</b> ${f.jumlah_dokter} • <b>Nakes:</b> ${f.jumlah_tenaga_kesehatan} • <b>Bed:</b> ${f.kapasitas_tempat_tidur}</div><div style="margin-top:6px"><b>Layanan:</b> ${escapeHTML(f.layanan || '-')}</div><div style="margin-top:6px"><b>Sumber:</b> ${escapeHTML(f.sumber_data)}</div><div class="popup-actions"><button type="button" onclick='setDestination(${f.latitude},${f.longitude},${JSON.stringify(f.nama)})'>Rute</button><button type="button" onclick='makeBuffer(${f.latitude},${f.longitude},${radius},${JSON.stringify(f.nama)})'>Buffer ${(radius/1000).toFixed(0)} km</button><a target="_blank" href="${gm}">Google Maps</a></div>`;
}
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  qs('#searchText').addEventListener('input',renderAll); qs('#kecFilter').addEventListener('change',renderAll); qs('#bpjsFilter').addEventListener('change',renderAll); qs('#igdFilter').addEventListener('change',renderAll);
  ...
  qs('#resetFilter').addEventListener('click',()=>{qs('#searchText').value=''; qs('#kecFilter').value=''; qs('#bpjsFilter').value=''; qs('#igdFilter').value=''; activeTypes=new Set([...new Set(facilities.map(f=>f.jenis))]); qsa('#typeFilters input').forEach(i=>i.checked=true); renderAll();});
  ...
  qs('#btnExportCsv').addEventListener('click',()=>download('fasilitas_kesehatan_purworejo.csv', toCSV(filtered().map(f=>({nama_faskes:f.nama,jenis_faskes:f.jenis,kecamatan:f.kecamatan,alamat:f.alamat,latitude:f.latitude,longitude:f.longitude,telepon:f.telepon,jam_layanan:f.jam_layanan,bpjs:f.bpjs,igd_24_jam:f.igd_24_jam,rawat_inap:f.rawat_inap,ambulans:f.ambulans,kapasitas_tempat_tidur:f.kapasitas_tempat_tidur,jumlah_dokter:f.jumlah_dokter,jumlah_tenaga_kesehatan:f.jumlah_tenaga_kesehatan,rating:f.rating,sumber_data:f.sumber_data,catatan:f.catatan}))), 'text/csv'));
